create database hm2806;
use hm2806;

drop table orders if exist;
...
SELECT title FROM products
LEFT JOIN orders ON products.id = orders.productid
WHERE orders.ordertype <> 'online' OR orders.id IS NULL;
...
GROUP BY customers.id, name, lastname
HAVING SUM(price * product_count) = (
SELECT MAX(total) FROM (
SELECT customerid, SUM(price * product_count) AS total FROM orders
JOIN products ON orders.productid = products.id
GROUP BY customerid
) AS subquery
);
